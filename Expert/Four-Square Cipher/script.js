...
function fourSquareEncrypt(key1,key2,message) {



    
if (key1 <= 25 && key1 >= 0 && key2 <= 25 && key2 >= 0 && message.length % 2 === 0) {
...
function fourSquareDecrypt(key1,key2,message) {

    
if (key1 <= 25 && key1 >= -25 && key2 <= 25 && key2 >= -25 && message.length % 2 === 0) {
...
for (let i = 0; i < message.length/2; i++) {
    ...
    for (let j = 0; j < square[0][0].length; j++) {
        for (let k = 0; k < square[0][0][j].length; k++) {
            ...
            if (square[0][1][j][k] === message[i * 2]) {
                X1 = k
                Y1 = j
            }
            if (square[1][1][j][k] === message[(i * 2) + 1]) {
                X2 = k
                Y2 = j
            }
    ...
    ParsedMessage += square[1][0][X2][Y1]  
    ParsedMessage += square[0][0][X1][Y2]  
}
...
console.log(fourSquareEncrypt(12,12,"wowowowo"))
console.log(fourSquareDecrypt(12,12,"umvdcfqcmu"))
